{- xmonad.hs
 - Author: Jupp Mueller ( jupp0r )
 - based on the config file by: Jelle van der Waa ( jelly12gen )
 -}
...
myLayoutHook  =  onWorkspace "1:chat" imLayout $  onWorkspace "2:web" webL $  onWorkspace "9:gimp" gimpL $ onWorkspace "6:vbox" fullL $ onWorkspace "6:games" fullL $ onWorkspace "8:vid" fullL $ standardLayouts 
   where
	standardLayouts =   avoidStruts  $ (tiled |||  reflectTiled ||| Mirror tiled ||| Grid ||| Full) 
        ...
        --Layouts
	tiled     = smartBorders (ResizableTall 1 (2/100) (toRational (2/(1+sqrt(5)::Double))) []) -- golden ratio
...
-------------------------------------------------------------------------------
---- Terminal --
myTerminalBlue :: String
myTerminalBlue = "rxvt -bg MidnightBlue -fg LightGray"

myTerminalRed :: String
myTerminalRed = "rxvt -bg firebrick -fg LightGray"

myTerminalGreen :: String
myTerminalGreen = "rxvt -bg DarkOliveGreen -fg LightGray"
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    [ ((modMask, xK_Return), spawn myTerminalGreen)
	, ((modMask .|. shiftMask, xK_Return), spawn myTerminalRed)
	, ((modMask .|. controlMask, xK_Return), spawn myTerminalBlue)
	...
	-- , ((modMask, xK_n ), windows W.focusRight)
    ...
    , ((modMask, xK_d ), sendMessage Expand)
    , ((modMask .|. shiftMask, xK_i ), sendMessage MirrorShrink)
    , ((modMask .|. shiftMask, xK_d ), sendMessage MirrorExpand)
    ...
    -- scratchpad
    , ((modMask , xK_grave), scratchpadSpawnAction defaultConfig  {terminal = myTerminalRed}) 
